...
sortModule = Module; // Module is exposed globally by sort.js

function sortArrayWithCpp() {
  const arr = array;
  const len = arr.length;
  const bytesPerElement = 4;
  const ptr = sortModule._malloc(len * bytesPerElement);

  sortModule.HEAP32.set(arr, ptr >> 2);
  sortModule._bubbleSort(ptr, len);

  const result = [];
  for (let i = 0; i < len; i++) {
    result.push(sortModule.HEAP32[(ptr >> 2) + i]);
  }

  sortModule._free(ptr);
  console.log("Sorted from C++:", result);
  renderBars(result);
}
